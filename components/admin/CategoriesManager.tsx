...
export default function CategoriesManager({ categories }: { categories: any[] }) {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    const payload = { name: name.trim(), slug: slug.trim() || slugify(name), description };
    try {
      ...
      if (res.ok) {
        toast.success(editing?.id ? "Kategori güncellendi" : "Kategori oluşturuldu");
        setIsOpen(false);
        router.refresh();
      } else {
  ...
  return (
    <div>
      <div className="mb-4 flex justify-end">
        <button onClick={openNew} className="px-3 py-2 bg-blue-600 rounded">Yeni Kategori</button>
      </div>
      ...
      {isOpen && (
        <form onSubmit={handleSave} className="mb-4 bg-white/5 p-4 rounded">
          <div className="grid gap-2">
            ...
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} className="p-2 bg-white/5 rounded" />
          ...
          {categories.map((c) => (
            <tr key={c.id} className="border-t border-zinc-800">
              ...
              <td className="px-4 py-3">
                <button onClick={() => openEdit(c)} className="text-sm text-blue-400 mr-3">Düzenle</button>
                <button onClick={() => handleDelete(c.id)} className="text-sm text-red-400">Sil</button>
              </td>
            </tr>
          ))}
        </tbody>
